...
string = "Does God play dice?"                             #The string
...
def main():
    ...
    c = Canvas(root, width = 2*h, height = 2*h)
    c.pack()
    ...
    def draw(c):                                           #Draws the gratings (with the basis in red only showing if basis_show is set to true)
        c.delete("all")
        dial(c)
        lines(c, grating, "#11f")
        if basis_show:
    ...
    def left(event):                                       #Anticlockwise (I think... do correct if I'm wrong)
        ...
        print(quantum_decode(quantum_encode(string, basis, perturbation), grating))#, end = "\r")
        draw(c)
    ...
    def click(event):                                       #Moveable dial - click, and the grating moves to point to your mouse
        ...
        os.system('cls' if os.name == 'nt' else "printf '\033c'")       #Clears a line
        print(quantum_decode(quantum_encode(string, basis, perturbation), grating))#, end = "\r")
        draw(c)
        
    def nothing(event):   #Doesn't affect grating angle: just iterates again
        os.system('cls' if os.name == 'nt' else "printf '\033c'")
        print(quantum_decode(quantum_encode(string, basis, perturbation), grating))#, end = "\r") #This has problems with some characters, so \r does not work. Looks pretty cool in terminal, though.
